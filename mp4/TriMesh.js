...
class TriMesh{
   ...
   */
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        for (const line of lines) {
            const words = line.split(/\b\s+(?!$)/);
            if (words[0] === 'v') {
                for (let i=1; i<=3; i++) {
                    this.vBuffer.push(parseFloat(words[i]));
                }
            } else if (words[0] === 'f') {
                for (let i=1; i<=3; i++) {
                    this.fBuffer.push(parseInt(words[i])-1);
                }
            }
        }
    ...
    */
    ...
    /**
    * Genrate a transformationmatrix this.modelMatrix=ST
    ...
    */
    ...
    canonicalTransform(){
        //Your code here
        const minX = this.minXYZ[0];
        const minY = this.minXYZ[1];
        const minZ = this.minXYZ[2];
        const maxX = this.maxXYZ[0];
        const maxY = this.maxXYZ[1];
        const maxZ = this.maxXYZ[2];
        this.modelMatrix = glMatrix.mat4.create();
        const L = Math.max(Math.max(maxX-minX, maxY-minY), maxZ-minZ);
        const scaleVec3 = glMatrix.vec3.fromValues(1/L, 1/L, 1/L);
        glMatrix.mat4.scale(this.modelMatrix, this.modelMatrix, scaleVec3);
        const translationVec3 = glMatrix.vec3.fromValues((-minX-maxX)/2, (-minY-maxY)/2, (-minZ-maxZ)/2);
        glMatrix.mat4.translate(this.modelMatrix, this.modelMatrix, translationVec3);
    }
